...
def backfill_purchase_chain_sales_partner(dry_run: int = 1):
    """
    Backfill row-level Sales Partner in sequence using document handlers
    (schema-safe across ERPNext versions):
      1) Material Request
      2) Purchase Order
      3) Purchase Receipt
      4) Purchase Invoice
    ...
    from it.handlers.sales_partner_dimension import (
        sync_material_request_sales_partner,
        sync_purchase_invoice_sales_partner,
        sync_purchase_order_sales_partner,
        sync_purchase_receipt_sales_partner,
    )
    ...
    result = {
        ...
        "counts": {
            "material_request_docs_changed": 0,
            "material_request_items_changed": 0,
            "material_request_header_changed": 0,
            "purchase_order_docs_changed": 0,
            "purchase_order_items_changed": 0,
            "purchase_receipt_docs_changed": 0,
            "purchase_receipt_items_changed": 0,
            "purchase_invoice_docs_changed": 0,
            "purchase_invoice_items_changed": 0,
        },
    }
    ...
    def _snapshot(doc):
        ...
        for d in (doc.get("items") or []):
            rows.append((d.name, (d.get("sales_partner") or "")))
        return {
            "header": doc.get("custom_sales_partner") if doc.doctype == "Material Request" else None,
            "rows": rows,
        }

    def _diff(before, after):
        b = dict(before.get("rows") or [])
        a = dict(after.get("rows") or [])
        row_changes = 0
        for key in set(list(b.keys()) + list(a.keys())):
            if (b.get(key) or "") != (a.get(key) or ""):
                row_changes += 1
        header_changed = (before.get("header") or "") != (after.get("header") or "")
        return row_changes, header_changed

    plans = [
        ("Material Request", sync_material_request_sales_partner, "material_request"),
        ("Purchase Order", sync_purchase_order_sales_partner, "purchase_order"),
        ("Purchase Receipt", sync_purchase_receipt_sales_partner, "purchase_receipt"),
        ("Purchase Invoice", sync_purchase_invoice_sales_partner, "purchase_invoice"),
    ]

    for doctype, fn, prefix in plans:
        ...
        for name in names:
            ...
            fn(doc)
            after = _snapshot(doc)
            row_changes, header_changed = _diff(before, after)

            if row_changes or header_changed:
                result["counts"][f"{prefix}_docs_changed"] += 1
                result["counts"][f"{prefix}_items_changed"] += row_changes
                if doctype == "Material Request" and header_changed:
                    result["counts"]["material_request_header_changed"] += 1
